...
1. **Neighbouring clubs are tied, not ranked.** Empirical-Bayes shrinkage puts between-club variance at 0.38 against within-club variance of 14.0, implying roughly **29% of the visible spread is noise**. Only the ends separate cleanly (Liverpool 8.26–9.56 versus Fulham 6.68–7.70). Read tiers.
2. **The qualifying list is fitted on the same season it is measured on.** Selecting the players out-of-sample drops measured stability from 0.41 to 0.40 here, so the leak is small — but in production the list should come from the previous season.
3. **The cutoff was not tuned, on purpose.** Sweeping it, 7+ CBIT/90 scores better than 6+. With only 20 clubs the standard error on these correlations is around 0.24, so that gap is inside the noise and picking the winner would be fitting to it.
4. **Part of this is just team quality** — the metric correlates +0.34 with the faced club's points. Not the whole story, but not a pure style signal either.
5. **Game state is invisible here.** Sides that lead sit deeper and their opponents chase, so the measure partly encodes how often a club was ahead. This dataset has no possession or game-state data to separate that from playing style.

**On splitting the table by home and away.** There is a real league-wide venue effect — a defender does about **0.7 more CBIT** away at a club than when hosting it — but it should not be split out, for three reasons that all point the same way. First, it *cancels*: every club plays exactly 19 home and 19 away, so the effect washes out of each club's average. Adjusting for it changes **0 of 20 ranks** and moves no club by more than 0.02 CBIT. Second, the club-by-club splits are not real. The observed variance of each club's home-minus-away gap is **0.433**, and pure sampling noise on samples this size already predicts **0.453** — so there is no detectable club-specific venue signal at all, and the eye-catching swings (Burnley 9.10 at home against 6.83 away) are what halving a noisy sample looks like. Testing each club against the *league* venue effect rather than against zero, only Burnley clears 95%, and roughly one false positive is expected across twenty tests. Third, splitting would halve every cell: measured separately, the home-only table has a reliability of **0.09** — indistinguishable from noise — against 0.41 for the combined one. The venue number is still worth knowing when you are weighing a single fixture; it is just a uniform adjustment, not a club trait.
