...
- **Goals per match are Poisson, not a bell curve.** The mean is 2.89 goals (SD 1.68), but the *shape* matters: variance ÷ mean = **0.98** where Poisson predicts exactly 1.00, and skew is +0.53 where a normal distribution requires 0. Fitting both across all 2,280 matches, Poisson is consistent with the data (χ² = 2.4, **p = 0.94**) while the normal curve is decisively rejected (χ² = 62.6, p < 0.001) — it even puts 2.2% of its mass on *negative* scorelines. Poisson fits better in five of six seasons; 2025/26 is a genuine exception, which is what a single 380-match season looks like when it has too little power to separate the two. **The payoff:** a clean sheet is just P(0) = e^−λ, so clean-sheet rate follows from goals conceded per game with nothing else fitted — accurate to **3.7 percentage points** across all 120 team-seasons here (Arsenal's 0.71 conceded/gm implies 49.1% clean sheets; they recorded 50.0%).
...
- **Distribution fits:** the normal and Poisson curves are both fitted to the observed goals-per-match counts and compared by Pearson chi-square, pooling the right tail until every expected count reaches 5. The normal density is integrated over each `[k−0.5, k+0.5]` interval so it is compared against integer counts on equal footing with Poisson's PMF. Because tail-pooling leaves the two models with different degrees of freedom (the normal fits two parameters, Poisson one), they are ranked by **p-value, not raw χ²**. Implemented in `analyze.py` with `math.erf`/`lgamma` and a hand-rolled incomplete gamma — this project has no scipy dependency.
