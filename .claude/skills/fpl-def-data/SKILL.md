...
**Hard gotcha — Understat blocks ALL server-side fetches, even from residential networks.** Understat serves a data-stripped page (no `statisticsData`) to any non-browser HTTP request — confirmed: it fails identically from a sandboxed IP, a home network, and a mobile hotspot. FBref (403), Sofascore (403), FotMob, and every public proxy/reader tried (jina, allorigins, codetabs, corsproxy) are also blocked. The worldfootballR GitHub mirror is stale (Understat complete only to 2023/24).

**Confirmed working workaround: fetch client-side from a real browser Console.** Navigating to an `understat.com` page in an actual browser DOES load `statisticsData` (confirmed in DevTools). The trick: `fetch()` calls from that page's Console still get the stripped response (Understat distinguishes real navigation from XHR/fetch) — so instead, load each club via a **hidden `<iframe>`** (a real navigation) and read `iframe.contentWindow.statisticsData` once it loads. This is what actually filled `docs/setpieces_goals.json` with real 2025/26 data. `fetch_setpiece_goals.py` documents the intent but can't run server-side; the browser-console iframe method is the one that works. Do NOT fabricate set-piece goals if regenerating and this method also fails — ask for a fresh Console snippet instead.
